...
import "./SelectionSeatsWagonType.css";
...
function SelectionSeatsWagonType({ className, onChange, value }) {
  ...
  const handleClick = (event) => {
    const { value } = event.target.closest(".selection-seats-wagon-type__item").dataset;
  ...
  return (
    <div className={cn("selection-seats-wagon-type", className)}>
      <p className="selection-seats-wagon-type__title">Тип вагона</p>
      <div className="selection-seats-wagon-type__list">
        {types.map((type) => {
          const Icon = type.icon;

          return (
            <button
              className="selection-seats-wagon-type__item"
              data-active={type.value === selectedType}
              data-value={type.value}
              key={type.value}
              onClick={handleClick}
            >
              <Icon className="selection-seats-wagon-type__icon" />
              <span className="selection-seats-wagon-type__text">{type.label}</span>
            </button>
          );
        })}
      </div>
...
SelectionSeatsWagonType.propTypes = {
  className: classNameType,
  onChange: PropTypes.func,
  value: PropTypes.oneOf(["first", "second", "third", "fourth"]),
};

export default SelectionSeatsWagonType;
